...
class BuildMatcher:
    ...
    def import_pob_code(self, pob_code: str, build_name: str = None) -> BuildRequirement:
        """
        ...
        try:
            ...
            if not build_name:
                ...
                if build_elem is not None:
                    build_name = build_elem.get('targetVersion', 'Imported Build')
                else:
            ...
            for affix in affix_matches:
                ...
                if affix_type == "life" and build.required_life > 0:
                    if affix.value and affix.value >= build.required_life * 0.7:  # 70% of requirement
                        score += 20
                        matched_requirements.append(f"Life: {affix.value} (need {build.required_life})")
                ...
                if affix_type == "energy_shield" and build.required_es > 0:
                    if affix.value and affix.value >= build.required_es * 0.7:
                        score += 20
                        matched_requirements.append(f"ES: {affix.value} (need {build.required_es})")
                ...
                if affix_type == "resistances":
                    for res_type, required in build.required_resistances.items():
                        if res_type in affix.mod_text.lower():
                            score += 10
                            matched_requirements.append(f"Resistance: {res_type}")
        ...
        for build in self.builds:
            if build.build_name == build_name:
                ...
                if build.main_skills:
                    lines.append(f"Main Skills: {', '.join(build.main_skills[:3])}")
