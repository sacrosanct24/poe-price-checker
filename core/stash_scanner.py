...
class StashItem:
    ...
    def __str__(self) -> str:
        return f"{self.name or self.type_line} @ Tab '{self.stash_tab_name}' ({self.position_x}, {self.position_y})"
...
class StashScanner:
    ...
    def _fetch_account_name(self) -> None:
        ...
        if not token:
            raise ValueError("Not authenticated - call oauth.authenticate() first")
    ...
    def get_stash_items(self, tab_index: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error("Failed to fetch items from tab %d: %s", tab_index, e)
            raise
    ...
    def scan_all_tabs(self) -> List[StashTab]:
        ...
        for tab_data in tabs_data:
            ...
            self.logger.info("Scanning tab %d: '%s' (%s)", tab_index, tab_name, tab_type)
        ...
        self.logger.info("Stash scan complete: %d tabs, %d total items", len(result), sum(len(t.items) for t in result))
    ...
    def scan_and_price(
            ...
            for item in tab.items:
                ...
                try:
                    ...
                    if results:
                        ...
                        item.chaos_value = float(best.get('chaos_value', 0) or 0)
                        item.divine_value = float(best.get('divine_value', 0) or 0)
        ...
        self.logger.info("Found %d valuable items (>= %.1fc)", len(valuable), min_chaos_value)
